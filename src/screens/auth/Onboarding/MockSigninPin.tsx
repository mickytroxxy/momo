...
const MockSigninPin = () => {
  ...
  const [signin, { error, data }] = useSignInDataMutation();
  const {fetchData} = useFetch();
  //console.log('Data-->> ', JSON.parse(data).Sessionid);
  ...
  const handleSuccessCallback = (Sessionid: string)=> {
    if(Sessionid != ''){
      // navigate
  ...
  const handleLogin = async () => {
    const payload = {"loginrequest": { "identity": item, "credential": { "secret": invalue, "type": "pincode" } } };
    ...
    <SafeAreaContainer>
      ...
      <Card
        ...
        <View style={{ marginTop: 'auto' }}>
          ...
          <Button disabled={!invalue} label="SIGN IN" onPress={() => {
              // signin({mobileNo: item, pin: invalue});
              // if(data.Sessionid != ''){
              //   console.log('Hiii')
              // }
              handleLogin();
